...
#add new files:dataloaders/__init__.py dataloaders/ffhq_ref_dataset.py  configs/train_yml/multi_step/VOSR_0.5B_ffhq_ref.yml
...
from models.refldm_vosr.eval_refaware import load_ref_tensor_from_row
...
patch_vosr_loss_return_extra(VOSR)
...
def main(config_path):
    ...
    if accelerator.is_main_process:
        ...
        # Optional identity metric for reference-aware evaluation (e.g., IDS_ref, IDS_gt)
        identity_metric = None
        if getattr(args, "identity_model_path", None):
            identity_metric = IdentityLoss(
                model_path=args.identity_model_path
            ).to(accelerator.device).eval()

            for p in identity_metric.parameters():
                p.requires_grad_(False)

            if accelerator.is_main_process:
                logger.info("[Eval IDS] Identity metric loaded for IDS_ref / IDS_gt")
    ...
    def get_ref_gate_stats(model):
        gates = []
        for name, p in model.named_parameters():
            if "ref_attn_gate" in name:
                gates.append(p.detach().float().view(-1).cpu())
        ...
        return {
            "ref_gate_mean": gates.mean().item(),
            "ref_gate_abs_mean": gates.abs().mean().item(),
            "ref_gate_min": gates.min().item(),
            "ref_gate_max": gates.max().item(),
        }

    # For debugging: track gradient norms of ref attention parameters
    def get_ref_grad_stats(model):
        grad_norms = []
        for name, p in model.named_parameters():
            if ("ref_cross_attn" in name or "ref_attn_gate" in name) and p.grad is not None:
                grad_norms.append(p.grad.detach().float().norm().cpu())
        ...
        return {
            "ref_grad_mean_norm": grad_norms.mean().item(),
            "ref_grad_max_norm": grad_norms.max().item(),
        }
    # Optimizer creation
    # params_to_optimize = model.parameters() [change to only optimize ref branch if specified]
    if getattr(args, "freeze_backbone", False) or getattr(args, "train_ref_only", False):
        trainable_keywords = [
            "ref_cross_attn",
            "ref_attn_gate",
        ]
        ...
        if accelerator.is_main_process:
            ...
            logger.info(
                f"[Freeze] Train ref branch only: "
    ...
    if args.dataset_type == 'txt':
        ...
        train_dataloader = DataLoader(
            ...
            persistent_workers=True if num_workers > 0 else False,
            prefetch_factor=prefetch_factor,
            collate_fn=filter_collate_fn,   # drop string metadata, keep tensors
        )
    ...
    refldm_addon = RefLDMVOSRStageEAddon(
        args=args,
        accelerator=accelerator,
    ...
    while global_step < args.max_train_steps:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if debug_this_step:
                if accelerator.is_main_process:
                    tqdm.write(f"[DEBUG] hq cpu shape={hq.shape}, dtype={hq.dtype}, "
            ...
            if debug_this_step:
                if accelerator.is_main_process:
                    tqdm.write(f"[DEBUG] hq moved to {hq.device}")
            ...
            with torch.no_grad():
                if debug_this_step:
                    if accelerator.is_main_process:
                        tqdm.write("[DEBUG] before venc preprocess")
                raw_image = (0.5 * lq + 0.5) * 255
                raw_image_ = preprocess_raw_image(raw_image, args)
                if debug_this_step:
                    if accelerator.is_main_process:
                        tqdm.write("[DEBUG] after venc preprocess")
                ...
                with accelerator.autocast():
                    if debug_this_step:
                        if accelerator.is_main_process:
                            tqdm.write("[DEBUG] before venc forward")
                    features, x_norm = unwrapped_venc.forward_with_features(raw_image_)
                    if debug_this_step:
                        if accelerator.is_main_process:
                            tqdm.write("[DEBUG] after venc forward")
            ...
            with accelerator.accumulate(model):
                ...
                with torch.no_grad():
                    ref_pack = refldm_addon.prepare_train_pack(
                        batch=batch,
                        hq_img_m11=hq,
                        lq_img_m11=lq,
                        lq_feature=z,
                ...
                print_gpu_mem("before_backward")

                # Backward and optimize
                accelerator.backward(loss_backward)

                print_gpu_mem("after_backward")

                if accelerator.sync_gradients and accelerator.is_main_process and global_step % 100 == 0:
                    unwrapped = accelerator.unwrap_model(model)

                    gate_stats = get_ref_gate_stats(unwrapped)
                    grad_stats = get_ref_grad_stats(unwrapped)

                    log_stats = {}
                    if gate_stats is not None:
                        log_stats.update(gate_stats)
                    if grad_stats is not None:
                        log_stats.update(grad_stats)

                    if len(log_stats) > 0:
                        logger.info(f"[Ref Monitor @ step {global_step}] {log_stats}")
                        accelerator.log({f"train/{k}": v for k, v in log_stats.items()}, step=global_step)
            ...
            if accelerator.sync_gradients:
                ...
                # Inference and logging at specified intervals
                if global_step % args.inference_steps == 1 and global_step > 0 and global_step != 1:
                    if accelerator.is_main_process:
                        model.eval()
                        ...
                        ids_ref_sum, ids_gt_sum = 0.0, 0.0
                        ids_ref_count, ids_gt_count = 0, 0

                        use_ffhq_csv_eval = getattr(args, "ffhq_ref_test_use_csv_for_eval", False)

                        with torch.no_grad():
                            # For FFHQ_Ref, we support two evaluation modes: (1) direct folder-based evaluation, where we compute metrics on all pairs of images in the specified HQ and LQ directories; (2) CSV-based evaluation, where we read a CSV file that specifies which HQ and LQ image pairs to evaluate. The CSV-based evaluation allows for more controlled and specific evaluation, especially if the dataset is large or if we want to focus on certain subsets of the data.
                            if use_ffhq_csv_eval:
                                ...
                                logger.info(
                                    ...
                                    f"Using FFHQ_Ref CSV eval: csv={args.ffhq_ref_test_csv}, "
                                    f"lq_dir={lq_dir}, hq_dir={hq_dir}"
                                )
                                ...
                                for _, row in eval_bar:
                                    ...
                                    if args.ae_type == "qwen":
                                        lq_latent = (unwrapped_model_ae.encode(lq_eval).latent_dist.sample() - latents_mean) * latents_std
                                        sr_m11 = (
                                        ...
                                        lq_latent = unwrapped_model_ae.encode(lq_eval).latent_dist.sample() * unwrapped_model_ae.config.scaling_factor
                                        sr_m11 = (
                                            vosr.sample_multistep_fm_ref(ema, lq_latent, n_steps=args.infer_steps, venc_fea=z.get('lq'), ref_fea=z.get('ref'))
                                            if isinstance(z, dict) and getattr(args, 'use_ref_attention', False)
                                            else vosr.sample_multistep_fm(ema, lq_latent, n_steps=args.infer_steps, venc_fea=z)
                                    ...
                                    if identity_metric is not None:
                                        sr_id_m11 = sr_01 * 2.0 - 1.0
                                        gt_id_m11 = gt_01 * 2.0 - 1.0

                                        with torch.no_grad():
                                            id_loss_gt = identity_metric(sr_id_m11.float(), gt_id_m11.float()).mean()
                                            ids_gt = 1.0 - id_loss_gt
                                            ids_gt_sum += float(ids_gt.item())
                                            ids_gt_count += 1

                                        ref_eval_m11, ref_names_for_ids = load_ref_tensor_from_row(
                                            row=row,
                                            hq_dir=hq_dir,
                                            to_tensor=to_tensor,
                                            device=accelerator.device,
                                        ...
                                        if ref_eval_m11 is not None:
                                            # ref_eval_m11: [1, K, 3, H, W]
                                            ref_flat = ref_eval_m11.squeeze(0)  # [K, 3, H, W]
                                            sr_rep = sr_id_m11.repeat(ref_flat.shape[0], 1, 1, 1)

                                            with torch.no_grad():
                                                id_loss_ref = identity_metric(sr_rep.float(), ref_flat.float()).mean()
                                                ids_ref = 1.0 - id_loss_ref
                                                ids_ref_sum += float(ids_ref.item())
                                                ids_ref_count += 1
                                    ...
                                    if accelerator.is_main_process and n_img < 3:
                                        logger.info(
                                            ...
                                            f"sr_01=({sr_01.min().item():.4f}, {sr_01.max().item():.4f}, mean={sr_01.mean().item():.4f})"
                                        )
                                ...
                                if args.test_lq_dir is None or args.test_gt_dir is None:
                                    ...
                                    logger.info(
                                        f"[Eval @ step {global_step}] "
                                        f"Scanning lq_dir={lq_dir}, gt_dir={gt_dir}"
                                    )
                        ...
                        if n_img > 0:
                            ...
                            ids_ref_avg = ids_ref_sum / max(ids_ref_count, 1)
                            ids_gt_avg = ids_gt_sum / max(ids_gt_count, 1)

                            logger.info(
                                f"[Eval @ step {global_step}] "
                                f"LPIPS: {lpips_avg:.4f}  "
                                f"MUSIQ: {musiq_avg:.4f}  "
                                f"IDS_ref: {ids_ref_avg:.4f}  "
                                f"IDS_gt: {ids_gt_avg:.4f}"
                            )
                            ...
                            accelerator.log(
                                {
                                    ...
                                    "eval/ids_ref": ids_ref_avg,
                                    "eval/ids_gt": ids_gt_avg,
                                },
                                step=global_step
                            )
                            ...
                            logger.warning(
                                f"[Eval @ step {global_step}] No valid eval image pairs found."
                            )
            ...
            if global_step % 50 == 0:
                logs = {
                    ...
                    "v_loss": loss_log_dict.get("loss_fm", loss.detach()).item()
                }
...
"""
   ...
   export CUDA_VISIBLE_DEVICES=3
     nohup torchrun --nproc_per_node=1 --master_port=29502 \
  train_vosr_refldm_modular.py \
  --config configs/train_yml/multi_step/ffhq_config/VOSR_0.5B_ffhq_stageE_ref_target_attention_extra.yml \
  > train_stageE2.log 2>&1 &
...
nohup torchrun --nproc_per_node=1 --master_port=29502 \
  ...
  > train_stageE_refonly_stage1.log 2>&1 &
...
nohup torchrun --nproc_per_node=1 --master_port=29502 \
  train_vosr_refldm_modular.py \
  --config configs/train_yml/multi_step/ffhq_config/test_stage2.yml \
  > train_stageE_refonly_stage2.log 2>&1 &
