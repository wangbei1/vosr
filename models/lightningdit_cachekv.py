...
from .refldm import cache_kv #add cache_kv
...
class Attention(nn.Module):
    ...
    Adds Ref-LDM style KV cache support:
      - cache_kv.mode == 'save': save current K/V
      - cache_kv.mode == 'use' : concat cached ref K/V into current K/V
    """
    def __init__(
        self,
        dim: int,
    ...
    def forward(self, x: torch.Tensor, rope=None) -> torch.Tensor:
        ...
        For this tensor layout, token dimension is -2.
        So cached K/V should be concatenated on dim=-2.
        """
        B, N, C = x.shape
        ...
        # ---------------- Ref-LDM style KV cache ----------------
        if cache_kv.mode == 'save':
            # detach to avoid keeping unnecessary graph from ref pass
            cache_kv.k[id(self)].append(k.detach())
            cache_kv.v[id(self)].append(v.detach())
            ...
            if len(kc_lst) > 0:
                # concat on token dimension
                k = torch.cat([k] + kc_lst, dim=-2)
                v = torch.cat([v] + vc_lst, dim=-2)
        # --------------------------------------------------------
...
class LightningDiT(nn.Module):
    ...
    def forward(self, x, t, r=None, z=None, is_ref: bool = False):
        """
        ...
        Args:
            ...
            t: (N,)
            r: (N,) or None
            z: list/tensor of visual conditions
            is_ref: whether this pass is the reference pass for KV cache saving
                    (currently only used for interface compatibility / debugging)
        """
    ...
    def forward_flexible(self, x, t, r=None, z=None, is_ref: bool = False):
        """
