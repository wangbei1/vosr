    ...
    def __getitem__(self, idx):
        for retry in range(self.max_retry):
            ...
            try:
                ...
                hq_tensor = self.to_tensor(gt_img)   # [C,H,W], 0~1
                ...
                if len(ref_names) == 0:
                    # 兜底：没有合法参考图时，直接用 gt 自己
                    ref_name = gt_name
                    ref_path = gt_path
                    ref_tensor = hq_tensor.clone()
                else:
                    ref_tensors = []
                    ref_paths = []
                    ...
                    if len(ref_tensors) == 0:
                        ref_tensors = [hq_tensor.clone()]
                        ref_paths = [gt_path]
                        ref_names = [gt_name]

                    use_multi_ref = getattr(self.args, "use_multi_ref", False)

                    if use_multi_ref:
                        sample["ref"] = torch.stack(ref_tensors, dim=0)  # [K,C,H,W]
                    else:
                        sample["ref"] = ref_tensors[0]                   # [C,H,W]
                    sample["ref_paths"] = ref_paths
                    sample["ref_names"] = ref_names

                # sample["ref_path"] = ref_path
                # sample["ref_name"] = ref_name
                # sample["ref_names"] = ref_names if len(ref_names) > 0 else [gt_name]

                if "lq_image" in self.df.columns and isinstance(row.get("lq_image", None), str):
                    lq_name = row["lq_image"]
                    sample["lq_name"] = lq_name
                    if self.lq_root is not None:
                        sample["lq_path"] = os.path.join(self.lq_root, lq_name)

                if getattr(self.args, "debug_dataset", False) and idx == 0:
                    print(f"[DATASET DEBUG] return keys: {list(sample.keys())}")
                    print(f"[DATASET DEBUG] hq shape: {sample['hq'].shape}")
                    print(f"[DATASET DEBUG] ref shape: {sample['ref'].shape}")
                ...
                if retry == self.max_retry - 1:
                    raise RuntimeError(
                        f"Failed to load FFHQRef data after {self.max_retry} retries. "
                        f"Last error: {str(e)}"
                    )
                continue
